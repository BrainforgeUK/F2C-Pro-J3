...
class Form2ContentControllerForm extends Form2ContentControllerFormBase
{
	...
	public function __construct($config = array())
	{
		...
		{
			$this->menuParams = $menu->getActive()->getParams();
			$this->editMode   = $this->menuParams->get('editmode', -1);
		}
	...
	function add()
	{
		...
		$contentTypeId = $this->menuParams->get('contenttypeid');
	...
	public function display($cachable = false, $urlparams = false)
	{
		...
		if ($this->editMode == F2C_EDITMODE_ALWAYS_CREATE_NEW || $this->editMode == F2C_EDITMODE_EDIT_EXISTING_OR_CREATE_NEW)
		{
			...
			switch ($this->menuParams->get('redirectmode'))
			{
				case 0:
					// custom url
					$errorRedirect = $this->menuParams->get('redirectaftersave');
					break;
				case 1:
	...
	public function save($key = null, $urlVar = null)
	{
		...
		if (parent::save($key, $urlVar))
		{
			if ($task != 'apply' && $this->editMode >= 0)
			{
				...
				if ($this->editMode == F2C_EDITMODE_ALWAYS_CREATE_NEW || $this->editMode == F2C_EDITMODE_EDIT_EXISTING_OR_CREATE_NEW)
				{
					switch ((int) $this->menuParams->get('redirectmode'))
					{
						case 0:
							// redirect to custom url
							$redirectLink = $this->menuParams->get('redirectaftersave', 'index.php');
							break;
						case 1:
