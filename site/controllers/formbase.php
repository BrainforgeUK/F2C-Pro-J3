...
{	
	...
	public function save($key = null, $urlVar = null)
	{
		if($result = parent::save($key, $urlVar))
		{
			...
			if($task == 'save2new')
			{
				$newTask = JFactory::getApplication()->isClient('site') ? 'form.add' : 'form.projectselect';
		...
		if(!empty($data))
		{
			...
			$formOld = $model->getItem($data['id']);
			
			foreach($contentType->fields as $field)
			{
				$field->prepareSubmittedData($data['id'], $formOld);
			}
			
           	// $contentType->fields is an id based array => convert it to an fieldname based array
            $fields = array();
            ...
            $data['fieldData'] = serialize($fields);
